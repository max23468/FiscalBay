...
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Milestone / task in esecuzione                              | M1 aperta dopo il via di fine M0 del 2026-09-23; M1-03 e M1-04 chiuse; M1-08 in corso |
...
| Prossima azione alla ripresa                                | M1-08: RuName e segreti eBay Production dopo l'accesso owner al portale, email di assistenza Google OAuth e conferma dell'arrivo su `info@` della risposta di prova; M1-05 richiede il mandato sul logo. Al riscontro eBay eseguire la checklist di M2-09 |
...
**Stato:** IN PROGRESS · **Prerequisiti:** M0 · **Contratto:** [§24](docs/MASTER_PLAN.md#s24)
...
**Prova posta umana del 2026-09-26:** un messaggio inviato dall'owner da `info@fiscalbay.it` via iCloud a Gmail risulta SPF pass (IP iCloud), DKIM pass con selettore `sig1` e dominio `fiscalbay.it`, DMARC pass con policy `quarantine` letta correttamente. Gmail lo ha comunque classificato come spam per somiglianza del contenuto (corpo «Test»), non per l'autenticazione; la reputazione del dominio nuovo resta da osservare con posta reale. Dal Gmail è stata inviata una risposta a `info@`.

**Aperti:** RuName eBay Production con callback `https://fiscalbay.it/api/auth/callback/ebay` e segreti `EBAY_CLIENT_ID`, `EBAY_CLIENT_SECRET`, `EBAY_RUNAME` sul Worker `fiscalbay`, bloccati dall'accesso al portale eBay Developers che richiede l'owner; email di assistenza utenti del branding Google OAuth, che resta l'email personale dell'owner perché Google ammette solo l'account amministratore o un suo gruppo e l'account non gestisce gruppi (serve un account o gruppo Google per `supporto@`); conferma owner dell'arrivo su `info@` della risposta inviata da Gmail; invio Auth Production verso destinatari non verificati, che richiede Workers Paid (attivo fino al 20 ottobre 2026); nome della D1 test `fiscalbay-m0-test`.
