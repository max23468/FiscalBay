...
- `src/fiscalbay/bot_dispatch.py`: dispatch sottile verso i domini bot.
- `src/fiscalbay/bot_{admin,orders,account,settings}.py`: logica bot per dominio.
...
- `src/fiscalbay/storage/`: connessione, schema e persistenza divisi per dominio.
- `src/fiscalbay/oauth_{rendering,callback,server}.py`: rendering, callback e HTTP OAuth.
