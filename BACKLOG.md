...
| Repository / branch / commit osservati nell’implementazione | `max23468/FiscalBay`; worktree `/Users/Matteo/Progetti/FiscalBay-m0`, branch `codex/adopt-2-0-m0`, ultimo commit verificato `1acd969`, base `origin/main` `ffc4f56` |
...
| Capacità account | PARZIALE                           | Cloudflare: la CLI autenticata il 2026-09-19 vede D1 `fiscalbay-m0-test` e le risorse CF-Ready già escluse dal perimetro. La D1 FiscalBay è vincolata alla giurisdizione UE e ha servito le prove dalla regione EEUR; nessun Worker `fiscalbay-m0` o Queue FiscalBay è presente. Il tentativo di leggere i deployment ha restituito il codice Cloudflare `10007`, Worker inesistente. Il ciclo concluso mostra 207.560 richieste Workers, 1,42 milioni CPU-ms, 20,27 milioni righe D1 lette, 27.890 scritte, 858 operazioni Queue, 24 operazioni R2 Class A e 43 Class B, tutte entro l'incluso e con costo 0 USD. Supabase: la Dashboard del 2026-09-13 conferma che l'organizzazione di Matteo è Free e condivide FiscalBay e SyncBay; uso osservato 0,83/5 GB egress, 293/500 MB database, 1/50.000 MAU e 0/1 GB Storage. Il progetto FiscalBay è stato ripristinato dalla pausa il 2026-09-19, senza costi; non è più il candidato runtime. SyncBay e Pratix sono rimasti attivi e invariati. |
| Polling/coda     | PASS parziale                      | Workers Queues è la coda scelta. Caso piano: 150 negozi, 12.000 cicli/giorno e 360.000 messaggi/mese; tre operazioni per messaggio producono circa 1,08 milioni di operazioni/mese prima dei retry. La prova di concorrenza Supabase resta evidenza storica del carico, mentre la prova Cloudflare remota attende una queue FiscalBay dedicata. Il binding non viene aggiunto finché esiste un consumer reale, per evitare configurazione vuota. Il readback eBay del 2026-09-19 assegna 5.000 chiamate/giorno a Trading `GetOrders`, insufficienti per 12.000 cicli/giorno. Il polling userà Fulfillment `getOrders`, il cui limite predefinito ufficiale è 100.000 chiamate/giorno; Developer Analytics non ha restituito la relativa risorsa Order, quindi il limite effettivo del keyset resta da attestare. |
...
| Recovery         | PASS mirato / BLOCKED operativo    | Il candidato usa D1 Time Travel, con retention nominale fino a 30 giorni sul piano Paid. Le tre migration applicative e Better Auth sono presenti nella D1 remota UE `fiscalbay-m0-test`. Una riga sintetica inserita dopo un bookmark è stata rimossa dal restore e le tre migration sono rimaste applicate. La prova Supabase con dump cifrato e round trip R2 resta evidenza storica sul dataset, ma non definisce il runbook del candidato. Restano da definire retention operativa, riconciliazione delle cancellazioni o revoche successive allo snapshot e drill conclusivo M9. |
