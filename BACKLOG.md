...
| Prossimo task eleggibile                                    | Completare il pairing Better Auth Infrastructure e il preflight dati, poi provare i quattro accessi senza acquisire ordini reali |
...
**Evidenza parziale:** `@better-auth/infra` 0.4.9 è compatibile con Better Auth 1.7.4 ed è configurato con il solo plugin `dash()`, condizionato alla chiave dedicata. La chiave non è nel repository ed è ora caricata come secret nel Worker test; Activity tracking, Sentinel, email, SMS e servizi enterprise restano disattivati. La dashboard non ha ancora creato il progetto né completato il pairing. La CLI `auth` 1.7.5 espone `login`, ma il comando pubblicato richiama ricorsivamente `npx auth@latest login`; l'esecuzione è stata interrotta e non ha prodotto sessioni o file. Sul dominio test, la sessione nulla e l'avvio dei provider Google ed eBay rispondono correttamente senza completare login o consenso.

**Blocco:** servono completamento del pairing Better Auth Infrastructure e prove reali controllate di linking, recupero, revoca, riuso sessione e MFA. La casella umana su iCloud Custom Email Domain non chiude da sola questo requisito. Origine HTTPS, client Google e RuName eBay 2.0 sono configurati; restano da provare i callback completi e il diritto email Identity. L'owner accetta il rischio sperimentale delle passkey per la qualifica.
