...
| Prossima azione alla ripresa                                | M1-08: decisione owner sul contenuto dell'apice e sul perimetro Production, email di assistenza Google OAuth e prova di posta umana; M1-05 richiede il mandato sul logo. Al riscontro eBay eseguire la checklist di M2-09 |
...
**Aperti:** contenuto dell'apice al posto dell'hosting Register finché la Production non è pubblicata (decisione owner; da esso dipendono la rimozione del record Register e l'URL about del RuName eBay); email di assistenza utenti del branding Google OAuth, che resta l'email personale dell'owner perché Google ammette solo l'account amministratore o un suo gruppo e l'account non gestisce gruppi (serve un account o gruppo Google per `supporto@`); prova di ricezione, risposta e allineamento DMARC della posta iCloud; configurazione e callback Production; nome della D1 test `fiscalbay-m0-test`.
