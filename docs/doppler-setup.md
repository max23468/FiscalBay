...
## Stato attuale repo
- `DOPPLER_PROJECT` impostato a: `FiscalBay`
...
### Come usarlo
- Il controllo `doppler-check` parte automaticamente sulle Pull Request verso `main`.
- In alternativa, puoi lanciarlo manualmente dalla pagina *Actions* con `workflow_dispatch`.
...
```bash
gh variable set DOPPLER_PROJECT --body "FiscalBay" --repo max23468/FiscalBay
gh variable set DOPPLER_CONFIG --body "production" --repo max23468/FiscalBay

gh secret set DOPPLER_TOKEN --body "<service_token_read_only>" --repo max23468/FiscalBay
```
