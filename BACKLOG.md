...
| Prossimo task eleggibile                                    | Rileggere billing Cloudflare e Stripe; scegliere o autorizzare il progetto Google dedicato a FiscalBay   |
| Repository / branch / commit osservati nell’implementazione | `max23468/FiscalBay`; worktree `/Users/Matteo/Progetti/FiscalBay-m0`, branch `codex/adopt-2-0-m0`, ultimo commit applicativo verificato `be701ff`, base `origin/main` `ffc4f56` |
| Blocchi noti iniziali                                       | Billing Cloudflare e Stripe non riletti; progetto Google FiscalBay, callback, mittente email, preflight dati e payload eBay sanificato mancanti |
| Materiale privato                                           | Inventario fuori checkout: riferimento locale `FiscalBay/m0-inventory` nella custodia Codex privata       |
| Operazioni remote parziali da riconciliare                  | Timer autodeploy 1.x riletto `disabled` / `inactive` il 2026-09-19; bot e callback 1.x attivi; nessun push, merge, deploy 2.0, nuovo provider o costo attivato |
| Prossima azione alla ripresa                                | Completare i readback billing Cloudflare e Stripe, poi il progetto Google dedicato e i prerequisiti callback/email |
...
| Candidato        | PROPOSTO / GATE ACCOUNT APERTI     | Il candidato tecnico ora preferito è Supabase Auth + Postgres, con dump logici cifrati conservati su Cloudflare R2 e un runner ancora da scegliere. La combinazione ha superato le prove discriminanti locali su passkey, RLS, concorrenza e ripristino. Workers Paid + D1 + Queues resta il confronto già provato; la scelta finale dipende dai quattro login reali, dai piani account, da eBay/Stripe e dall'approvazione del modello di recovery.                                                                                                                    |
| Costi nominali   | DA APPROVARE                       | Workers Paid: minimo 5 USD/mese per account; 10 milioni richieste e 30 milioni CPU-ms/mese inclusi. Email Sending: 3.000 invii/mese inclusi, poi 0,35 USD/1.000. Queue: 1 milione operazioni/mese incluse, poi 0,40 USD/milione. Supabase Pro parte da 25 USD/mese e include un progetto Micro tramite 10 USD di crediti compute e backup giornalieri con 7 giorni di retention; PITR parte da circa 100 USD/mese e richiede almeno compute Small. R2 Standard include nominalmente 10 GB-mese, 1 milione di operazioni Class A, 10 milioni Class B ed egress senza costo; oltre soglia si applica il consumo. Nessun costo è stato attivato. Il 2026-09-13 la Dashboard indicava Workers Paid in scadenza e R2 Paid attivo, entrambi con data 2026-09-20; lo stato successivo a tale data richiede un nuovo readback. Prezzi Stripe Managed Payments e piani effettivi degli account condivisi restano da leggere sull'account. |
| Capacità account | PARZIALE                           | Cloudflare: la CLI autenticata il 2026-09-19 vede 2 D1, 4 queue e 2 bucket R2, tutti denominati per CF-Ready e quindi esclusi dal perimetro FiscalBay. Nessuna risorsa FiscalBay è presente. La Dashboard del 2026-09-13 mostrava R2 a 3,43 MB, 12 operazioni Class A, 16 Class B e costo 0 USD nel periodo; billing e capacità fatturabile correnti restano aperti. Supabase: la Dashboard del 2026-09-13 conferma che l'organizzazione di Matteo è Free e condivide FiscalBay e SyncBay; uso osservato 0,83/5 GB egress, 293/500 MB database, 1/50.000 MAU e 0/1 GB Storage, con periodo di tolleranza scaduto. Il progetto FiscalBay esistente era healthy su nano, senza repository, migration o backup, con zero richieste nell'ultima ora; non è stato modificato. L'altra organizzazione Free contiene Pratix e usava 32/500 MB database. |
| Polling/coda     | PASS parziale                      | Caso piano: 150 negozi, 12.000 cicli/giorno e 360.000 messaggi/mese. A tre operazioni per messaggio sono 1.080.000 operazioni/mese prima di retry: supera Free e usa l’incluso Paid più circa 0,03 USD/mese. Il readback eBay del 2026-09-19 assegna 5.000 chiamate/giorno a Trading `GetOrders`, tutte residue: non sostiene 12.000 cicli/giorno. Il polling deve usare Fulfillment `getOrders`, il cui limite predefinito ufficiale è 100.000 chiamate/giorno; Developer Analytics non ha restituito la relativa risorsa Order, quindi il limite effettivo del keyset resta da attestare. Trading può restare solo per chiamate mirate dimostrate necessarie. |
...
| Stripe           | PASS documentale / BLOCKED account | Managed Payments è in public preview, supporta aziende italiane e opera come MoR per categorie ammesse. Sandbox non prova ricevute email o comparsa in Link. L’account Standard letto via CLI non espone eligibility/attivazione Managed Payments: serve readback Dashboard, senza transazione live.                                                                                                                                                                                                                                    |
...
**Blocco:** inventario privato e tool disponibili; mancano endpoint HTTPS test, destinatario/mittente email e sessioni console controllate. Prosegue la sola prova locale senza dati reali.
...
**Blocco:** Supabase è stato riletto in Dashboard; il piano Free e la capacità condivisa osservata sono riportati sopra. Cloudflare CLI è autenticata e ha escluso le risorse CF-Ready dal perimetro FiscalBay, ma non attesta piano, billing o capacità residua fatturabile. Il piano Workers risultava in scadenza il 2026-09-20 e richiede un readback Dashboard corrente. Nessun upgrade eseguito.
...
**Blocco:** servono origine HTTPS controllata, Google dedicato, scope e RuName eBay destinati alla 2.0 e trasporto email per linking, recupero, revoca, riuso sessione e MFA effettivi. Google Cloud è autenticato, ma l'inventario completo mostra soltanto quattro progetti estranei e nessun progetto FiscalBay; nessuna risorsa è stata creata o riusata. eBay Developers è autenticato e il keyset FiscalBay è identificato, ma il callback e lo scope Identity della 2.0 non sono configurati. L'owner accetta il rischio sperimentale delle passkey per la qualifica.
...
**Blocco:** matrice documentale avviata; la CLI conferma soltanto l’account Standard e la Dashboard richiede login. Eligibility, attivazione, condizioni e prezzi Managed Payments richiedono una sessione Dashboard controllata. Nessuna transazione o catalogo creato.
...
**Blocco:** nessun dato reale è stato acquisito. L'account eBay non è esentato dalle notifiche di cancellazione marketplace e non ha endpoint, verification token o email configurati; questo requisito va chiuso prima della readiness live. Servono inoltre condizioni effettive Stripe, dati dell’operatore e perimetro del trattamento prima delle prove su ordini reali.
...
**Blocco:** slice locale ordine→D1→pagina, Auth e permessi nel commit `d62e053` è eseguibile e testata; login reale→consenso seller→ordine richiede la chiusura dei gate provider e trattamento sopra. HTTP locale della pagina minima: 200; nessun segreto nel client.
...
**Blocco:** Supabase Auth + Postgres con backup cifrato su R2 è il candidato tecnico proposto dopo le prove discriminanti locali; Cloudflare Workers + D1 resta il confronto qualificato. Nell'account Cloudflare non esistono risorse FiscalBay e le sole D1, queue e bucket appartengono a CF-Ready. Il via owner di fine M0 verrà richiesto dopo i readback account e costi correnti, i quattro accessi reali, la prova eBay read-only, Managed Payments, recovery operativa e vincoli legali preliminari. M1 resta ferma.
