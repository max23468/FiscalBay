...
| Prossimo task eleggibile                                    | Autenticare Cloudflare, Stripe ed eBay; scegliere o autorizzare il progetto Google dedicato a FiscalBay  |
...
| Blocchi noti iniziali                                       | Cloudflare, Stripe ed eBay non autenticati; progetto Google FiscalBay, callback, mittente email e dati legali mancanti |
...
| Prossima azione alla ripresa                                | Rileggere timer e Git; completare i tre login console mancanti e l'inventario Google dedicato             |
...
**Blocco:** Supabase è stato riletto in Dashboard; il piano Free e la capacità condivisa osservata sono riportati sopra. Cloudflare richiede ancora login e la CLI non attesta piano, billing o capacità residua fatturabile. Nessun upgrade eseguito.
...
**Blocco:** servono origine HTTPS controllata, Google ed eBay autorizzati e trasporto email per linking, recupero, revoca, riuso sessione e MFA effettivi. Google Cloud è autenticato, ma l'inventario completo mostra soltanto quattro progetti estranei e nessun progetto FiscalBay; nessuna risorsa è stata creata o riusata. eBay Developers richiede login. L'owner accetta il rischio sperimentale delle passkey per la qualifica.
...
**Blocco:** Supabase Auth + Postgres con backup cifrato su R2 è il candidato tecnico proposto dopo le prove discriminanti locali; Cloudflare Workers + D1 resta il confronto qualificato. Il via owner di fine M0 verrà richiesto dopo i readback account e costi, i quattro accessi reali, eBay, Managed Payments, recovery operativa e vincoli legali preliminari. M1 resta ferma.
