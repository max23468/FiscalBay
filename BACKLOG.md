...
**Evidenza locale del 2026-09-26:** registro tipizzato degli errori con codice stabile, stato HTTP e retryability; `Retry-After` solo dove applicabile. Il Worker genera un correlation ID per richiesta, presente nelle risposte e nei log strutturati. I log applicativi ammettono solo codice, operazione, stato e correlation ID, senza messaggi grezzi, URL, header, CF o token. Better Auth non usa il logger predefinito; gli errori server passano dal gestore React Router e gli Auth 5xx sono registrati con metadati ammessi. UI e messaggi IT/EN con i18next e react-i18next, route `/en`, formatter UTC/locale. `pnpm verify` verde con 34 test; prova HTTP sul Worker locale compilato: `/` e `/en` HTTP 200 con lingua corretta, `/en/missing` HTTP 404 in inglese, tutti con correlation ID. D1 locale sintetica, nessuna scrittura o readback remoto in questo task.
