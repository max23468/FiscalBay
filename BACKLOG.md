...
| Prossimo task eleggibile                                    | M1-08 indipendente; M1-05 in corso: icona base approvata, wordmark e varianti dopo il via owner. M2-09 attende il riscontro eBay `260920-000007` |
...
| Prossima azione alla ripresa                                | Procedere con M1-08; M1-05: wordmark e varianti dopo il via owner. Al riscontro eBay eseguire la checklist di M2-09 |
...
**Avanzamento del 2026-09-26:** mandato owner ricevuto. Una prima vettorializzazione fedele al concept, con bordo destro continuo e icona generica senza tile iOS, è stata superata dall'indicazione owner di migliorare il logo senza vincolo di fedeltà (D141). Dopo quattro giri di proposte (tessera sola, tessera con F, chip realistico e astratto) l'owner ha approvato l'icona base: tessera inclinata di 8° con cornice blu `#1A4FA6` 600×460 raggio 104, card `#F5F8FD` 480×340 raggio 44 con angoli concentrici, chip giallo pieno astratto a sinistra come sulla tessera e tre righe blu, verde e rossa in proporzione 5:4:3 con margini uguali (`assets/brand/fiscalbay-icon.svg`). Per 16–32 px c'è una versione dritta con cornice e righe più spesse (`assets/brand/fiscalbay-icon-small.svg`), perché l'inclinazione sfoca i bordi a quelle dimensioni. Il giallo del chip resta `#F6C12B` con contrasto 1,57:1 sulla card: il 3:1 richiederebbe un senape `#B38708` e il chip si riconosce per forma e posizione. Prossimo passo, dopo il via owner: wordmark con `Bay` blu, logo orizzontale, chiaro/scuro, monocromo, favicon e avatar Telegram.
