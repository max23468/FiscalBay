...
| Repository / branch / commit osservati nell’implementazione | `max23468/FiscalBay`; checkout `/Users/Matteo/Progetti/FiscalBay`; `main` al commit `b2d9144` dopo il cutover M1-00, `develop` avanza con i task M1; 1.x congelata in `legacy/1.x` (`508ded8`). Il worktree M0 è stato rimosso dopo l'assorbimento |
...
| Prossima azione alla ripresa                                | Proseguire M1 dai task eleggibili; al riscontro eBay eseguire la checklist di M2-09 |
