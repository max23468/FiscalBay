...
| Prossima azione alla ripresa                                | M1-08: RuName e segreti eBay Production dopo l'accesso owner al portale, email di assistenza Google OAuth e prova di posta umana; M1-05 richiede il mandato sul logo. Al riscontro eBay eseguire la checklist di M2-09 |
...
**Aperti:** RuName eBay Production con callback `https://fiscalbay.it/api/auth/callback/ebay` e segreti `EBAY_CLIENT_ID`, `EBAY_CLIENT_SECRET`, `EBAY_RUNAME` sul Worker `fiscalbay`, bloccati dall'accesso al portale eBay Developers che richiede l'owner; email di assistenza utenti del branding Google OAuth, che resta l'email personale dell'owner perché Google ammette solo l'account amministratore o un suo gruppo e l'account non gestisce gruppi (serve un account o gruppo Google per `supporto@`); prova di ricezione, risposta e allineamento DMARC della posta iCloud; invio Auth Production verso destinatari non verificati, che richiede Workers Paid (attivo fino al 20 ottobre 2026); nome della D1 test `fiscalbay-m0-test`.
