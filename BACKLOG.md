...
| Repository / branch / commit osservati nell’implementazione | `max23468/FiscalBay`; worktree `/Users/Matteo/Progetti/FiscalBay-m0`, branch `codex/adopt-2-0-m0`, ultimo commit verificato `853a22d`, base `origin/main` `ffc4f56` |
...
| Alternative      | PARZIALE                           | Python, Telegram-first e VPS 1.x sono incompatibili con la baseline. Supabase resta ammissibile: la passkey sperimentale ha superato la prova locale e il limite di recovery Free può essere coperto tecnicamente da dump cifrati fuori sede, se l'owner approva la deroga. Nessun adapter alternativo è stato implementato.                                                                                                                                                                                                                                                        |
| Candidato        | DA DECIDERE                        | Tre configurazioni restano nel confronto esplicito. Proposta principale: Supabase Pro in organizzazione dedicata per Auth, Postgres, Cron, Queues ed Edge Functions, con Cloudflare per frontend statico, DNS e R2. Alternativa economica: Workers Paid + D1 + Queues + Better Auth. Supabase Free + R2 resta utile per M0/test ma non è proposto per la produzione, a causa di pausa automatica e margine database limitato. Le prove locali favoriscono Supabase per RLS/Auth e Cloudflare per costo e recovery nativa. La slice Supabase nel checkout è una prova del candidato, non approvazione architetturale; D1 e Better Auth restano verificabili nella storia Git senza mantenere due runtime concorrenti. Servono le prove live Auth/eBay e la decisione owner su costo e recovery. |
| Costi nominali   | DA APPROVARE                       | Workers Paid: minimo 5 USD/mese per account; 10 milioni richieste e 30 milioni CPU-ms/mese inclusi. Per 360.000 messaggi/mese, tre operazioni Queue per messaggio valgono circa 1,08 milioni di operazioni prima dei retry: circa 0,03 USD oltre il milione incluso. Supabase Pro parte da 25 USD/mese per un'organizzazione e include 10 USD di crediti compute, sufficienti a un progetto Micro; mantenendo anche SyncBay nella stessa organizzazione, due progetti Micro porterebbero il totale nominale a circa 35 USD/mese. Un'organizzazione Pro dedicata a FiscalBay evita di cambiare piano agli altri progetti e resta nominalmente a 25 USD/mese. Include backup giornalieri con 7 giorni di retention; PITR ha un costo molto superiore e non è proposto per l'avvio. R2 Standard include nominalmente 10 GB-mese, 1 milione di operazioni Class A, 10 milioni Class B ed egress senza costo. Nessun costo è stato attivato. Il readback Cloudflare del 2026-09-19 conferma Workers Paid `In scadenza` il 2026-09-20, mentre R2 Paid resta `attivo`; il ciclo 2026-08-20..2026-09-19 si è chiuso a 0 USD. La Dashboard Stripe indica per Managed Payments un sovrapprezzo del 3,5% per transazione; il costo totale dipende anche dai servizi Stripe sottostanti. |
| Capacità account | PARZIALE                           | Cloudflare: la CLI autenticata il 2026-09-19 vede 2 D1, 4 queue e 2 bucket R2, tutti denominati per CF-Ready e quindi esclusi dal perimetro FiscalBay. Nessuna risorsa FiscalBay o Worker `fiscalbay-m0` è presente. Il ciclo concluso mostra 207.560 richieste Workers, 1,42 milioni CPU-ms, 20,27 milioni righe D1 lette, 27.890 scritte, 858 operazioni Queue, 24 operazioni R2 Class A e 43 Class B, tutte entro l'incluso e con costo 0 USD. Dopo la cessazione Paid, Workers Free offre nominalmente 100.000 richieste dinamiche/giorno e 10 ms CPU per invocazione; gli asset statici sono gratuiti e senza limite di richieste, ma SSR/Auth richiede una prova remota mirata perché Cloudflare indica 10-20 ms come fascia tipica dei carichi più pesanti. Supabase: la Dashboard del 2026-09-13 conferma che l'organizzazione di Matteo è Free e condivide FiscalBay e SyncBay; uso osservato 0,83/5 GB egress, 293/500 MB database, 1/50.000 MAU e 0/1 GB Storage, con periodo di tolleranza scaduto. Edge Functions Free include 500.000 invocazioni/mese, 150 s di wall clock, 2 s CPU e 256 MB per istanza. Il progetto FiscalBay, prima healthy e vuoto, il 2026-09-19 risultava `INACTIVE` per la pausa automatica Free; l'API ufficiale lo ha ripristinato senza costo sulla stessa versione `17.6.1.166` e lo stato finale è `ACTIVE_HEALTHY`. Sul piano Free il progetto può tornare in pausa dopo sette giorni di bassa attività, quindi la continuità operativa resta un rischio da risolvere nella scelta del piano. SyncBay e Pratix sono rimasti attivi e invariati. |
| Polling/coda     | PASS parziale                      | Caso piano: 150 negozi, 12.000 cicli/giorno e 360.000 messaggi/mese. La prova locale con Supabase CLI 2.117.0, `pg_cron` 1.6.4 e `pgmq` 1.5.1 ha inserito 360.000 messaggi piccoli in 2,07 s; la coda attiva occupava 62 MB. Cron ogni minuto è stato creato, riletto attivo e rimosso; due lettori concorrenti hanno ottenuto messaggi distinti. Un'invocazione Edge al minuto vale circa 43.200 invocazioni/mese prima dei retry, entro le 500.000 Free, e può acquisire in media 8,3 cicli per invocazione. Il readback eBay del 2026-09-19 assegna 5.000 chiamate/giorno a Trading `GetOrders`, tutte residue: non sostiene 12.000 cicli/giorno. Il polling deve usare Fulfillment `getOrders`, il cui limite predefinito ufficiale è 100.000 chiamate/giorno; Developer Analytics non ha restituito la relativa risorsa Order, quindi il limite effettivo del keyset resta da attestare. |
| D1               | PASS locale                        | Dataset sintetico: 70.000 ordini, 210.000 righe articolo, 84.000 identificativi fiscali, 14.000 grant e 10 negozi. SQLite locale compattato: 62.181.376 byte; inserimento bulk circa 0,94 s DB / 4,23 s comando. Query pagina tenant/grant da 50 ordini: 20–68 ms in due esecuzioni locali. Test Workerd verdi per isolamento tenant, più identificativi e consumo quota atomico.                                                                                                                                                      |
| Auth             | PASS locale / BLOCKED live         | Better Auth genera e usa schema D1 senza deriva; email/password crea un utente non verificato, passkey richiede sessione, Google ed eBay avviano solo i provider previsti. Nello spike temporaneo fuori checkout, Supabase CLI 2.117.0, GoTrue 2.196.0 e `supabase-js` 2.116.0 hanno completato registrazione e nuovo accesso WebAuthn in Chromium con autenticatore virtuale e RP ID `localhost`. La stessa passkey ha autenticato di nuovo dopo un restore pulito di Auth e dati. Il rischio accettato per la qualifica resta la stabilità dell'API sperimentale. Supabase Auth supporta provider OAuth2 personalizzati, fino a tre sul piano Free. Per eBay il mapping può usare i campi Identity di primo livello `userId` come `sub` e `username` come nome, con `email_optional=true`; FiscalBay richiede poi un'email verificata nel prodotto, come previsto dal Master Plan, senza richiedere l'email eBay e senza adapter userinfo. Restano da provare sul provider reale la stabilità di `userId`, il callback e il linking. Il readback remoto Supabase mostra Auth ancora vergine: `site_url` locale, nessuna allowlist redirect, signup ed email attivi con conferma richiesta, Google, linking manuale e passkey disattivati, nessun RP ID/origine WebAuthn e nessun SMTP. Il progetto Google dedicato `fiscalbay-2-0-max23468` è attivo senza billing, API o credenziali; i quattro progetti preesistenti sono rimasti invariati. Lo scope login eBay Identity resta distinto dallo scope seller. Mancano callback applicativo HTTPS, RuName/scopi 2.0, mittente email e prove live di linking, recupero, revoca e MFA. |
...
| Recovery         | PASS locale / BLOCKED operativo    | Supabase ha prodotto in 2,88 s un dump `auth,public` del dataset M0; schema, dati e ruoli compressi e cifrati con `age` occupano 17.382.383 byte. Upload e download su R2 locale con Wrangler 4.131.1 sono risultati identici per SHA-256; un target Supabase pulito ha ripristinato in 3,29 s 10 utenti, 1 credenziale WebAuthn, 70.000 ordini, 210.000 articoli, 84.000 identificativi e 14.001 grant. RLS, quote e passkey sono rimasti validi. Il dump dati generico include tabelle Storage gestite e fallisce su `storage.buckets_vectors`; il comando qualificato deve limitarsi a `--schema auth,public` e conservare il `setval` Auth. Il dump ruoli contiene grant gestiti dal provider e non va applicato come schema applicativo. Restano da approvare deroga, runner, custodia chiave, bucket/retention reali, piano e riconciliazione di cancellazioni o revoche successive allo snapshot. Se FiscalBay userà Supabase Storage, gli oggetti richiederanno una copia separata. Questa prova è locale e mirata; il drill conclusivo resta M9. |
| Toolchain        | PASS                               | Node 26.8.2 e pnpm 12.4.1 sono entrambi attivati da `mise`; è stata rimossa la caduta silenziosa sul pnpm 11 del runtime Codex. TypeScript 7.0.2, React/DOM 19.3.0, React Router 8.3.1, Vite 8.3.0, Wrangler 4.131.1, Vitest 4.1.11, Supabase CLI 2.117.0 e `supabase-js` 2.116.0. Install frozen, typecheck, 4 test applicativi, 10 test pgTAP, lint database, build e verifica documentale sono verdi con il candidato Supabase. Il plugin Vitest Cloudflare e Better Auth sono stati rimossi insieme a D1 perché non hanno più un consumatore nel candidato. |
...
**Evidenza DNS/TLS:** il 2026-09-19 il pannello Register.it autenticato mostra 14 record con TTL 900 e ultimo aggiornamento 2026-09-09 10:32. L'apex usa `A 195.110.124.133`, `www` e `ftp` sono CNAME verso l'apex; `test.fiscalbay.it` non esiste. I nameserver sono `ns1.register.it` e `ns2.register.it`; DNSSEC è disattivato. La posta ordinaria usa `MX 10 mail.register.it`, SPF `include:spf.webapps.net`, `_autodiscover._tcp`, `authsmtp`, `autoconfig` e `pop`; il sottodominio `pec` ha MX `mx.pec-email.com` e SPF dedicato. Apex e `www` rispondono HTTP 200 da Apache con una pagina statica, mentre HTTPS presenta un certificato `*.dadapro.com` privo di SAN FiscalBay e fallisce la verifica. Il dominio è attivo fino al 2027-09-09; non risultano hosting attivi, account email creati o PEC attiva. L'email inclusa e la PEC omaggio risultano ancora da attivare. Il pannello avverte che spostare gli MX fuori da Register.it per almeno sette giorni elimina caselle, alias, inoltri e relativi dati: un eventuale cambio DNS deve conservare esplicitamente tutti i record posta. Nessun record, nameserver, account o contenuto è stato modificato.

**Blocco:** il controllo DNS è disponibile, ma servono destinazione tecnica, record e certificato valido per `test.fiscalbay.it`, oltre a una casella e un mittente controllati. Creare client OAuth o credenziali legate a URL provvisori resta sospeso fino alla definizione del callback.
...
**Evidenza parziale:** Supabase è stato riletto in Dashboard e via CLI; il piano Free e la capacità condivisa osservata sono riportati sopra. La pausa automatica del progetto FiscalBay è stata rilevata e il progetto è tornato `ACTIVE_HEALTHY` sulla stessa versione `17.6.1.166`, senza costo e senza toccare SyncBay o Pratix. Cloudflare CLI ha escluso le risorse CF-Ready dal perimetro FiscalBay e confermato l'assenza di un Worker M0 già distribuito. La Dashboard ha confermato piani, scadenze, utilizzo e costo del ciclo: Workers Paid cessa il 2026-09-20, R2 Paid resta attivo e tutto l'uso osservato è rimasto entro l'incluso.
...
**Evidenza parziale:** la documentazione e l'implementazione corrente di Supabase Auth confermano provider OAuth2 personalizzati sul piano Free, PKCE configurabile, linking manuale e passkey sperimentali. eBay può usare un provider `custom:` senza un secondo Auth: `userId` e `username` sono attributi Identity di primo livello, mentre `email_optional=true` consente di omettere l'email eBay. FiscalBay richiede separatamente un'email verificata nel prodotto, senza inventare l'attributo e senza introdurre un adapter userinfo. La configurazione remota Supabase è stata riletta e non contiene ancora provider Google/eBay, passkey, RP ID, redirect di test o SMTP.
...
**Evidenza parziale:** oltre alla misura D1 riportata sopra, lo spike PostgreSQL ha caricato 70.000 ordini, 210.000 articoli, 84.000 identificativi fiscali e 14.000 grant su 10 workspace. Il database occupava 113.519.763 byte; gli inserimenti principali hanno richiesto circa 1,05 s per gli ordini, 3,39 s per gli articoli, 0,99 s per gli identificativi primari, 0,18 s per i secondari e 0,31 s per i grant. La Data API con RLS ha restituito 7.000 ordini per ciascun owner e zero righe cross-tenant. Due RPC concorrenti su una quota residua hanno prodotto un solo grant; il tentativo cross-tenant ha lasciato il contatore a zero. Lo schema ora vive nella migrazione Supabase del candidato con grant espliciti, RLS su tutte le tabelle esposte e RPC `security definer` con `search_path` vuoto. Dopo un reset pulito, 10 test pgTAP hanno confermato privilegi, isolamento, CF bloccati, accesso dopo grant, quota e negazione cross-tenant; `supabase db lint` non ha trovato errori. Gli advisor dello spike non avevano segnalato warning, anche dopo il restore. Chiusura sospesa fino ai prerequisiti account/eBay e alla misura finale di raw temporaneo, retention e picchi.
...
**Evidenza:** pin e lockfile del candidato, configurazione `mise` per Node 26.8.2 e pnpm 12.4.1, configurazione Workers tipizzata, install frozen, typecheck, 4 test applicativi, 10 test pgTAP, lint database, build React Router e documentazione verdi. `supabase-js` 2.116.0 è usato direttamente dalla slice. Vitest 4.1.11 resta il runner dell'export; il plugin Cloudflare è stato rimosso insieme a D1 perché non ha più un consumatore.
...
**Evidenza parziale:** la slice mantenuta nel commit `853a22d` usa Supabase Auth e Data API con la sola chiave publishable nel client. Nel browser locale un account controllato ha completato login email/password, lettura di due ordini sintetici tramite RLS, oscuramento iniziale degli identificativi fiscali e sblocco del primo ordine tramite RPC; il CF autorizzato è apparso e il pulsante del secondo ordine è diventato disabilitato a quota esaurita. I test pgTAP verificano anche isolamento cross-tenant e assenza del privilegio client di inserire grant. Build e gate locali sono verdi. D1 e Better Auth sono stati rimossi dal runtime per evitare due candidati mantenuti in parallelo.

**Blocco:** login reale→consenso seller→ordine richiede la chiusura dei gate provider e trattamento sopra. Google/eBay, email remota, passkey remota e seller OAuth non sono configurati; nessun segreto è presente nel client.
...
**Blocco:** la proposta principale da sottoporre all'owner è Supabase Pro in un'organizzazione dedicata, con Cloudflare per frontend statico, DNS e R2; l'alternativa è Workers Paid + D1 + Queues + Better Auth. Supabase Free è limitato a M0/test. La proposta Supabase privilegia RLS, Auth integrata, PostgreSQL e minore codice di sicurezza applicativo; Cloudflare privilegia costo, singolo piano da circa 5 USD e Time Travel D1 nativo fino a 30 giorni. Nell'account Cloudflare non esistono risorse FiscalBay e le sole D1, queue e bucket appartengono a CF-Ready. Restano discriminanti la prova reale dei quattro login, l'Identity eBay custom su Supabase, il callback seller e una misura del frontend statico rispetto a Workers SSR. Restano inoltre da approvare costo, recovery, runner del dump R2, email e configurazioni Stripe. Il via owner di fine M0 verrà richiesto dopo la prova eBay read-only, la preparazione callback, Managed Payments e i vincoli legali preliminari. M1 resta ferma.
