...
**Stato corrente:** M0 avviata il 2026-09-13 sulla baseline approvata. Le prove locali indipendenti sono concluse; i gate che richiedono console, account e callback controllati restano circoscritti e bloccati.
...
| Campo                                                       | Stato osservato il 2026-09-13                                                                             |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
...
| Prossima azione alla ripresa                                | Rileggere timer e Git; completare Auth/eBay/Stripe e piani account con sessioni controllate               |
...
| Trigger 1.x      | PASS circoscritto                  | GitHub non espone webhook di repository o workflow di deploy. Sulla sola VPS autorizzata, hostname `fiscalbay-bot`, `fiscalbay-autodeploy.timer` era attivo su `main`; è stato disabilitato e riletto `disabled` / `inactive`. Bot e callback 1.x sono rimasti attivi. Rilettura obbligatoria prima di push o merge.                                                                                                                                                                                                                    |
| Alternative      | PARZIALE                           | Python, Telegram-first e VPS 1.x sono incompatibili con la baseline. Supabase resta nella selezione attiva: soltanto Supabase Free senza deroga è incompatibile con il requisito corrente di recovery nativa. Nessun adapter alternativo è stato implementato.                                                                                                                                                                                                                                                                           |
| Candidato        | PROVVISORIO / CONFRONTO RIAPERTO  | La slice esistente prova Cloudflare Workers Paid + D1 + Queues + Email Sending con Better Auth. Supabase Auth + Postgres è di nuovo candidato effettivo e richiede uno spike mirato sui quattro login e una misura PostgreSQL/RLS, senza costruire una seconda applicazione completa.                                                                                                                                                                                                                                                   |
| Costi nominali   | DA APPROVARE                       | Workers Paid: minimo 5 USD/mese per account; 10 milioni richieste e 30 milioni CPU-ms/mese inclusi. Email Sending: 3.000 invii/mese inclusi, poi 0,35 USD/1.000. Queue: 1 milione operazioni/mese incluse, poi 0,40 USD/milione. Supabase Pro parte da 25 USD/mese e include un progetto Micro tramite 10 USD di crediti compute e backup giornalieri con 7 giorni di retention; PITR parte da circa 100 USD/mese e richiede almeno compute Small. Prezzi Stripe Managed Payments e piani effettivi degli account condivisi restano da leggere sull'account. |
| Capacità account | PARZIALE                           | Lettura CLI privata: account Cloudflare già usato da altri progetti, 2 D1, 4 queue, nessun bucket R2 e nessun dominio Email Sending. Le quote nominali coprono la misura dati; piano e capacità fatturabile residua non sono attestati dalla CLI. Supabase espone 3 progetti sani su 2 organizzazioni; l'API backup risponde per tutti e tre con PITR disabilitato, ma piano e capacità fatturabile residua non sono leggibili dalla CLI.                                                                                                                                                             |
| Polling/coda     | PASS documentale                   | Caso piano: 150 negozi, 12.000 cicli/giorno e 360.000 messaggi/mese. A tre operazioni per messaggio sono 1.080.000 operazioni/mese prima di retry: supera Free e usa l’incluso Paid più circa 0,03 USD/mese. Quote eBay/eventi reali restano da misurare.                                                                                                                                                                                                                                                                               |
...
| Auth             | PASS locale / BLOCKED live         | Better Auth genera e usa schema D1 senza deriva; email/password crea un utente non verificato, passkey richiede sessione, Google ed eBay avviano solo i provider previsti. Supabase Auth offre passkey WebAuthn first party sperimentali. Uno spike temporaneo fuori checkout con CLI 2.117.0, GoTrue 2.196.0 e `supabase-js` 2.116.0 ha creato e confermato un utente locale, aperto una sessione e ottenuto da `auth.passkey.startRegistration()` challenge, opzioni WebAuthn e RP ID `localhost`; l'utente è stato eliminato e lo stack arrestato. Il rischio accettato per la qualifica è la stabilità dell'API; resta da provare la cerimonia browser completa. Lo scope login eBay Identity resta distinto dallo scope seller. Mancano callback HTTPS, RuName/keyset autorizzato, Google, mittente email e prove live di linking, recupero, revoca e MFA. |
| eBay             | PASS documentale / BLOCKED account | Identity può supportare il login, ma campi e scope riservati richiedono approvazione eBay. `buyer.taxIdentifier` viene da `getOrder`, non da `getOrders`; più identificativi sono conservati. Mancano quote/keyset, payload sanificato autorizzato, test read-only e contratto notifiche cancellazione sull’account effettivo.                                                                                                                                                                                                          |
...
| Recovery         | QUALIFICATA PARZIALMENTE           | D1 Free offre 7 giorni Time Travel; Paid 30 giorni. Supabase Free non offre backup automatici, ma supporta dump logici CLI/`pg_dump` off site; questi richiedono una deroga alla regola corrente sulle sole protezioni native. Supabase Pro include backup giornalieri nativi con 7 giorni di retention; PITR è un add-on distinto. I backup DB non includono gli oggetti Storage. La scelta deve ancora coprire cancellazioni/revoche successive allo snapshot.                                                                                                                                                      |
| Toolchain        | PASS                               | Node 26.8.2, pnpm 12.4.1, TypeScript 7.0.2, React/DOM 19.3.0, React Router 8.3.1, Vite 8.3.0, Wrangler 4.131.1, Vitest 4.1.11 per compatibilità col plugin Cloudflare, Oxlint/Oxfmt. Install frozen, peer check, typecheck, test e build locali qualificati.                                                                                                                                                                                                                                                                            |
...
**Blocco:** risorse e consumi CLI inventariati; le sessioni web non sono autenticate e le CLI non attestano piano, billing o capacità residua fatturabile. Nessun upgrade eseguito.
...
**Evidenza parziale:** schema e avvii Better Auth locali dei quattro metodi sono provati. Sul percorso Supabase è riuscito lo start della registrazione passkey con sessione Auth e RP ID controllato; lo spike non ha creato progetti remoti o costi. L'avvio iniziale dell'intero stack locale ha esaurito la memoria del container Analytics; il secondo avvio limitato a database, Kong e Auth è riuscito con servizi healthy.

**Blocco:** servono la cerimonia WebAuthn completa su browser/origine test, Google ed eBay autorizzati e trasporto email per linking, recupero, revoca, riuso sessione e MFA effettivi. L'owner accetta il rischio sperimentale delle passkey per la qualifica.
...
**Blocco:** fonti ufficiali e modello multi-identificativo acquisiti; mancano keyset/RuName/scope approvati, quote condivise, account controllato e lettura sanificata autorizzata.
...
**Blocco:** budget puro polling/coda calcolato sopra; disponibilità eventi, paginazione, backfill, retry e margine dipendono dalle quote eBay del keyset M0-05.
...
**Evidenza parziale:** misura 70.000+ e test D1 riportati sopra; D1 non espone una Data API diretta al client. Chiusura sospesa fino ai prerequisiti account/eBay e alla misura finale di raw temporaneo, retention e picchi.
...
**Blocco:** matrice documentale avviata; la CLI conferma soltanto l’account Standard. Eligibility, attivazione, condizioni e prezzi Managed Payments richiedono una sessione Dashboard controllata. Nessuna transazione o catalogo creato.
...
**Blocco:** restano ammissibili D1 con Time Travel e Supabase Pro con backup giornalieri nativi; Supabase Free richiede una deroga esplicita per usare dump logici esterni come recovery. Servono piano account effettivo, costo accettabile, percorso Auth/email, copertura separata degli oggetti Storage e fonte riconciliabile delle cancellazioni successive allo snapshot.
...
**Evidenza:** pin e lockfile del candidato, configurazione Workers tipizzata, install frozen, peer check, typecheck, test Workerd e build React Router verdi. Vitest 4.1.11 è l’ultima 4.x compatibile con `@cloudflare/vitest-plugin` 1.1.8, che richiede `^4.1.0`; Vitest 5 è stato escluso dopo peer check.
...
**Blocco:** nessun dato reale è stato acquisito. Servono condizioni effettive eBay/Stripe, dati dell’operatore e perimetro del trattamento prima delle prove account.
...
**Blocco:** il candidato Cloudflare dispone già di slice locale, mentre Supabase è stato riaperto e richiede soltanto le prove discriminanti su Auth/passkey, PostgreSQL/RLS, recovery e costo. Il via owner di fine M0 verrà richiesto dopo questo confronto, i readback account, i quattro accessi, eBay, Managed Payments e vincoli legali preliminari. M1 resta ferma.
