...
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Milestone / task in esecuzione                              | M1 aperta dopo il via di fine M0 del 2026-09-23; M1-03 in integrazione |
| Prossimo task eleggibile                                    | M1-03 e M1-04 dopo M1-01; M1-05 in parallelo con mandato di rifinitura del logo; M1-08 indipendente. M2-09 attende il riscontro eBay `260920-000007` |
| Repository / branch / commit osservati nell’implementazione | `max23468/FiscalBay`; il 2026-09-26 `main` è `4df1961`, `develop` è `aa96d65` al primo deploy CI test verificato; 1.x congelata in `legacy/1.x` (`508ded8`) |
...
| Trigger 1.x      | PASS circoscritto                  | GitHub non espone webhook di repository o workflow di deploy. Sulla sola VPS autorizzata, hostname `fiscalbay-bot`, `fiscalbay-autodeploy.timer` era attivo su `main`; è stato disabilitato e riletto `disabled` / `inactive`, da ultimo il 2026-09-26. Anche il servizio è `inactive`. Bot e callback 1.x sono rimasti attivi. Rilettura obbligatoria prima di push o merge.                                                                                                                                                                                            |
...
**Stato:** IN PROGRESS · **Prerequisiti:** M0, M1-01 · **Contratto:** [§27](docs/MASTER_PLAN.md#s27) · [§29](docs/MASTER_PLAN.md#s29)
...
**Evidenza parziale del 2026-09-26:** la migration D1 `0006` aggiunge unicità dell'appartenenza, vincoli sul ciclo/dato fiscale, grant immutabile e venti posti lifetime atomici. Preflight test: zero appartenenze duplicate e zero grant Free senza dato. Migration applicata a `fiscalbay-m0-test`; readback: zero migration pendenti, tabella, tre trigger e due indici presenti. `pnpm verify` locale verde, 30 test inclusi isolamento, concorrenza e rollback; `https://test.fiscalbay.it/` risponde 200. La modifica al servizio di sblocco resta da integrare e distribuire nel Worker test; nessuna scrittura Production.
