...
**Evidenza trasporto Auth:** Cloudflare Email Service è disponibile in beta e non risultano domini di invio già configurati nell'account. Sul piano Free consente senza costo invii verso destinatari verificati, sufficienti per la prova controllata M0; l'invio a destinatari arbitrari richiede Workers Paid. Il binding nativo è predisposto con mittente limitato a `accesso@auth.fiscalbay.it`, separando SPF/DKIM transazionali dalla posta iCloud sull'apice. Better Auth usa il binding per verifica indirizzo e recupero password, senza API key o nuova dipendenza. Il gate completo e il dry run Wrangler sono verdi: pacchetto compresso 642,89 KiB, D1 e binding email riletti, origine `https://test.fiscalbay.it`, Custom Domain unico e `workers.dev` disattivato. L'onboarding DNS e una consegna reale restano sospesi finché la zona non è attiva su Cloudflare.
