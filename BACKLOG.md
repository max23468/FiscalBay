...
| Candidato        | PROPOSTO / GATE ACCOUNT APERTI     | Il candidato tecnico preferito è Supabase centrale per Auth, Postgres, Cron, Queues ed Edge Functions, con Cloudflare per frontend e dump logici cifrati su R2. La combinazione ha superato prove locali su passkey, RLS, concorrenza, coda, ripristino e una slice applicativa autenticata. Il runtime mantenuto nel checkout usa ora soltanto Supabase; D1 e Better Auth restano evidenza storica del confronto, senza una seconda implementazione concorrente. La scelta finale dipende da hosting frontend, quattro login reali, eBay/Stripe e approvazione del modello di recovery.                                                                                                                    |
| Costi nominali   | DA APPROVARE                       | Workers Paid: minimo 5 USD/mese per account; 10 milioni richieste e 30 milioni CPU-ms/mese inclusi. Email Sending: 3.000 invii/mese inclusi, poi 0,35 USD/1.000. Queue: 1 milione operazioni/mese incluse, poi 0,40 USD/milione. Supabase Pro parte da 25 USD/mese e include un progetto Micro tramite 10 USD di crediti compute e backup giornalieri con 7 giorni di retention; PITR parte da circa 100 USD/mese e richiede almeno compute Small. R2 Standard include nominalmente 10 GB-mese, 1 milione di operazioni Class A, 10 milioni Class B ed egress senza costo; oltre soglia si applica il consumo. Nessun costo è stato attivato. Il readback Cloudflare del 2026-09-19 conferma Workers Paid `In scadenza` il 2026-09-20, mentre R2 Paid resta `attivo`; il ciclo 2026-08-20..2026-09-19 si è chiuso a 0 USD. La Dashboard Stripe indica per Managed Payments un sovrapprezzo del 3,5% per transazione; il costo totale dipende anche dai servizi Stripe sottostanti. |
...
**Blocco:** Supabase centrale con backup cifrato su R2 e Cloudflare per il frontend è il candidato tecnico proposto dopo le prove discriminanti locali; Cloudflare Workers + D1 resta il confronto qualificato. Nell'account Cloudflare non esistono risorse FiscalBay e le sole D1, queue e bucket appartengono a CF-Ready. Restano da decidere Workers Free contro riattivazione Paid dopo una misura SSR/Auth, piano Supabase Free contro Pro per margine database e recovery, runner del backup, email e configurazioni Auth/eBay/Stripe. Il via owner di fine M0 verrà richiesto dopo la prova eBay read-only, la preparazione callback, Managed Payments e i vincoli legali preliminari. M1 resta ferma.
