...
| Prossimo task eleggibile                                    | Completare il preflight dati e predisporre un destinatario email controllato, poi provare i quattro accessi senza acquisire ordini reali |
...
| Blocchi noti iniziali                                       | Attivazione e prova Stripe non autorizzate; destinatario email controllato, preflight dati, diritto email Identity e payload eBay sanificato mancanti |
| Materiale privato                                           | Inventario fuori checkout: riferimento locale `FiscalBay/m0-inventory` nella custodia Codex privata       |
| Operazioni remote parziali da riconciliare                  | Timer autodeploy 1.x riletto `disabled` / `inactive` il 2026-09-19; bot e callback 1.x attivi. Creato il progetto Google dedicato `fiscalbay-2-0-max23468`, senza billing; branding test e client web `FiscalBay Test` configurati con callback dedicato. Ripristinato il progetto Supabase Free FiscalBay dalla pausa automatica, senza costo; stato finale riletto `ACTIVE_HEALTHY`. Creata la D1 temporanea di test `fiscalbay-m0-test` con giurisdizione UE, applicate tre migration e completato un restore Time Travel con sola riga sintetica. La delega autorevole di `fiscalbay.it` è attiva su Cloudflare dal 2026-09-20. Il Worker `fiscalbay-test` è distribuito sul solo Custom Domain `test.fiscalbay.it`, con `workers.dev` disattivato e sette segreti runtime custoditi da Cloudflare. Email Sending è attivo su `auth.fiscalbay.it`, con record SPF, DKIM e DMARC pubblicati; nessun messaggio inviato. L'owner dichiara completata la configurazione iCloud Custom Email Domain con `info@fiscalbay.it` e `supporto@fiscalbay.it`; non è stata ripetuta una verifica esterna. eBay ha salvato il RuName Production dedicato `FiscalBay Test` sul keyset `botCF`, con privacy, about e callback su `test.fiscalbay.it`, e vi ha spostato OAuth come richiesto dall'owner. Il reload finale mostra OAuth disattivato sul RuName `FiscalBay` 1.x e attivo su `FiscalBay Test`. Better Auth Infrastructure ha creato e collegato il progetto `FiscalBay Test` sul piano Starter gratuito; il dashboard riletto mostra zero utenti. Nessun push o merge 2.0 eseguito |
| Prossima azione alla ripresa                                | Completare il preflight del trattamento e scegliere un destinatario controllato; verificare quindi login, email e callback reali senza acquisire ordini |
...
| Capacità account | PARZIALE                           | Workers Free offre 100.000 richieste dinamiche/giorno, 10 ms CPU per invocazione, 50 subrequest esterne per invocazione, 200.000 eventi log/giorno con 3 giorni di retention e asset statici senza limite di richieste. Queues Free include 10.000 operazioni/giorno e 24 ore di retention. D1 Free include 5 milioni di righe lette/giorno, 100.000 scritte/giorno e 5 GB totali. I limiti sono condivisi dall'account con CF-Ready: il ciclo precedente, ancora Paid, mostrava 207.560 richieste Workers, 1,42 milioni CPU-ms, 20,27 milioni righe D1 lette, 27.890 scritte e 858 operazioni Queue nel mese, ma il Free richiede letture giornaliere. La CLI vede D1 `fiscalbay-m0-test`, il Worker `fiscalbay-test` e le risorse CF-Ready; nessuna Queue FiscalBay è ancora presente. |
...
| Auth             | PASS deploy/pairing / BLOCKED login reali | Better Auth genera e usa schema D1 senza deriva; email/password crea un utente non verificato, passkey richiede sessione, Google ed eBay avviano solo i provider previsti. I token OAuth sono cifrati in D1 e le route HTTP che restituiscono access token o refresh token sono chiuse; il rinnovo resta disponibile al codice server. Il Worker test risponde via HTTPS con sessione nulla e avvia Google ed eBay verso i rispettivi endpoint ufficiali, con callback, state e scope previsti; nessun consenso o callback reale è stato completato. Better Auth Infrastructure è collegato al progetto `FiscalBay Test` sul piano Starter gratuito e legge configurazione e statistiche con zero utenti; Sentinel e gli altri servizi restano disattivati. Nello spike temporaneo fuori checkout, Supabase CLI 2.117.0, GoTrue 2.196.0 e `supabase-js` 2.116.0 hanno completato registrazione e nuovo accesso WebAuthn in Chromium con autenticatore virtuale e RP ID `localhost`. La stessa passkey ha autenticato di nuovo dopo un restore pulito di Auth e dati. Il rischio accettato per la qualifica resta la stabilità dell'API sperimentale. Il client Google dedicato e il RuName eBay 2.0 sono collegati agli URL di test. Mancano diritto email Identity e prove live di linking, recupero, revoca e MFA. |
...
**Blocco:** servono un destinatario email controllato e il preflight del trattamento prima di prove con account o dati reali. Il trasporto transazionale Better Auth resta separato dalla casella iCloud; l'invio pubblico resta vincolato al checkpoint Workers Paid.
...
**Criterio di completamento:** Confronto con l’originale approvato dall’owner; nessuna rigenerazione respinta viene promossa a riferimento canonico. Dopo l’approvazione, sostituire il logo 1.0 provvisorio nel branding Google Auth e aggiornare il logo dei keyset eBay pertinenti.
