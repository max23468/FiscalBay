...
**Evidenza DNS/TLS:** il 2026-09-19 `fiscalbay.it` risolve a `195.110.124.133`, `www` è un CNAME verso l'apex e `test.fiscalbay.it` non risolve. La delega pubblicata dal registro `.it`, riletta anche dopo l'avvio del cambio su Register.it, indica ancora `ns1.register.it` e `ns2.register.it`; MX e SPF esistenti puntano a Register.it, ma l'owner conferma che non esistono caselle attive da preservare. Apex e `www` rispondono HTTP 200 da Apache con una pagina statica, mentre HTTPS presenta un certificato `*.dadapro.com` privo di SAN FiscalBay e fallisce la verifica. La posta umana userà iCloud Custom Email Domain; non serve importare messaggi o indirizzi precedenti.

**Evidenza trasporto Auth:** Cloudflare Email Service è disponibile in beta e non risultano domini di invio già configurati nell'account. Sul piano Free consente senza costo invii verso destinatari verificati, sufficienti per la prova controllata M0; l'invio a destinatari arbitrari richiede Workers Paid. Il binding nativo è predisposto con mittente limitato a `accesso@auth.fiscalbay.it`, separando SPF/DKIM transazionali dalla posta iCloud sull'apice. Better Auth usa il binding per verifica indirizzo e recupero password, senza API key o nuova dipendenza. L'onboarding DNS e una consegna reale restano sospesi finché la zona non è attiva su Cloudflare.
