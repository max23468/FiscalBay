...
**Evidenza DNS/TLS:** il 2026-09-19 `fiscalbay.it` risolve a `195.110.124.133`, `www` è un CNAME verso l'apice e `test.fiscalbay.it` non risolve. Register.it ha registrato alle 20:06:02 i nameserver `carl.ns.cloudflare.com` e `pat.ns.cloudflare.com`; entrambi rispondono autorevolmente per la zona con i record temporanei corretti. Tutti i server autoritativi `.it` interrogati continuano però a delegare verso `ns1.register.it` e `ns2.register.it`, quindi la migrazione resta in propagazione e non è ancora attestata. MX e SPF esistenti puntano a Register.it, ma l'owner conferma che non esistono caselle attive da preservare. Apex e `www` rispondono HTTP 200 da Apache con una pagina statica, mentre HTTPS presenta un certificato `*.dadapro.com` privo di SAN FiscalBay e fallisce la verifica. La posta umana userà iCloud Custom Email Domain; non serve importare messaggi o indirizzi precedenti. Il candidato è configurato per pubblicare soltanto `test.fiscalbay.it` come Custom Domain, senza endpoint `workers.dev`; l'origine locale resta esplicita nella configurazione di sviluppo.
