...
| Repository / branch / commit osservati nell’implementazione | `max23468/FiscalBay`; worktree isolato, branch `codex/adopt-2-0-m0`, base `origin/main` `ffc4f56`         |
...
| Alternative      | ESCLUSE / NON SCELTE               | Python, Telegram-first e VPS 1.x sono incompatibili con la baseline. Supabase Free è incompatibile con recovery nativa e capacità account non attestata; Supabase Auth mantiene inoltre aperto il rischio passkey. Nessun adapter alternativo è stato implementato.                                                                                                                                                                                                                                                                     |
| Candidato        | PROPOSTO                           | Cloudflare Workers Paid + D1 + Queues + Email Sending, una sola app React Router/TypeScript e Better Auth. Riduce provider e usa trigger/constraint D1 per i grant. Richiede il via owner solo dopo la chiusura dei gate sotto.                                                                                                                                                                                                                                                                                                         |
| Costi nominali   | DA APPROVARE                       | Workers Paid: minimo 5 USD/mese per account; 10 milioni richieste e 30 milioni CPU-ms/mese inclusi. Email Sending: 3.000 invii/mese inclusi, poi 0,35 USD/1.000. Queue: 1 milione operazioni/mese incluse, poi 0,40 USD/milione. Prezzi Stripe Managed Payments e attribuzione del canone Cloudflare all’account condiviso restano da leggere sull’account.                                                                                                                                                                             |
| Capacità account | PARZIALE                           | Lettura CLI privata: account Cloudflare già usato da altri progetti, 2 D1, 4 queue, nessun bucket R2 e nessun dominio Email Sending. Le quote nominali coprono la misura dati; piano e capacità fatturabile residua non sono attestati dalla CLI. Supabase espone 3 progetti sani su 2 organizzazioni, piano non leggibile dalla CLI.                                                                                                                                                                                                   |
...
| Auth             | PASS locale / BLOCKED live         | Better Auth genera e usa schema D1 senza deriva; email/password crea un utente non verificato, passkey richiede sessione, Google ed eBay avviano solo i provider previsti. Lo scope login eBay Identity resta distinto dallo scope seller. Mancano callback HTTPS, RuName/keyset autorizzato, Google, mittente email e prove live di linking, recupero, revoca e MFA.                                                                                                                                                                   |
...
| Recovery         | QUALIFICATA CON COSTO              | D1 Free offre 7 giorni Time Travel; Paid 30 giorni. Supabase Free non offre backup automatici e può sospendere progetti inattivi. Il candidato richiede Workers Paid; cancellazioni/revoche successive allo snapshot richiederanno un registro esterno riconciliabile, da definire prima della chiusura M0-09.                                                                                                                                                                                                                          |
...
**Blocco:** schema e avvii locali dei quattro metodi sono provati. Servono origini/callback test, Google ed eBay autorizzati e trasporto email per linking, recupero, revoca, riuso sessione e MFA effettivi.
...
**Blocco:** Free escluso e D1 Paid proposto; servono piano account effettivo, percorso Auth/email e fonte riconciliabile delle cancellazioni successive allo snapshot.
...
**Blocco:** slice locale ordine→D1→pagina, Auth e permessi è eseguibile e testata; login reale→consenso seller→ordine richiede la chiusura dei gate provider e trattamento sopra. HTTP locale della pagina minima: 200; nessun segreto nel client.
...
**Blocco:** candidato e costi nominali sono proposti sopra; il via owner di fine M0 verrà richiesto dopo i readback account, i quattro accessi, eBay, Managed Payments e vincoli legali preliminari. M1 resta ferma.
