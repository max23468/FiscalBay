...
| Scelta candidato Cloudflare | REGISTRATO 2026-09-19 | L'owner sceglie Workers Paid + D1 + Queues + Better Auth perché Supabase Pro non rientra nel budget. La scelta approva l'assetto e il relativo ordine di costo, senza attivare rinnovi, risorse o deploy |
...
| Candidato        | SCELTO / GATE PROVIDER APERTI      | L'owner ha scelto Workers Paid + D1 + Queues + Better Auth il 2026-09-19. La slice D1 e Better Auth torna a essere il solo runtime mantenuto. Supabase Pro è escluso perché non rientra nel budget; Supabase Free non offre la continuità operativa richiesta e resta soltanto evidenza storica delle prove M0. Restano da chiudere i quattro login reali, callback seller eBay, trasporto email e trattamento dei dati prima di concludere M0. |
| Costi nominali   | SCELTI / ATTIVAZIONE DA VERIFICARE | Workers Paid: minimo 5 USD/mese per account; 10 milioni richieste e 30 milioni CPU-ms/mese inclusi. Per 360.000 messaggi/mese, tre operazioni Queue per messaggio valgono circa 1,08 milioni di operazioni prima dei retry: circa 0,03 USD oltre il milione incluso. L'ordine di costo scelto è quindi circa 5,03 USD/mese prima di retry, email e consumi oltre soglia. R2 Standard include nominalmente 10 GB-mese, 1 milione di operazioni Class A, 10 milioni Class B ed egress senza costo. Nessun costo è stato attivato in questa decisione. Il readback Cloudflare del 2026-09-19 mostrava Workers Paid `In scadenza` il 2026-09-20 e R2 Paid `attivo`; lo stato corrente del rinnovo va riletto prima di qualsiasi azione di fatturazione. La Dashboard Stripe indica per Managed Payments un sovrapprezzo del 3,5% per transazione; il costo totale dipende anche dai servizi Stripe sottostanti. |
...
**Evidenza parziale:** la slice locale ordine→D1→pagina, Better Auth e permessi del commit `d62e053` è stata ripristinata come unico runtime mantenuto dopo la scelta owner del 2026-09-19. HTTP locale della pagina minima: 200; nessun segreto nel client. La slice Supabase del commit `853a22d` resta disponibile nella storia Git come prova M0, senza mantenere due architetture concorrenti.
...
**Evidenza parziale:** l'owner ha scelto Workers Paid + D1 + Queues + Better Auth il 2026-09-19, accettando l'ordine di costo di circa 5 USD/mese più consumo marginale. Supabase Pro è escluso per budget e Supabase Free per la continuità insufficiente. Cloudflare offre il costo più basso tra i candidati qualificati, D1 Time Travel fino a 30 giorni sul piano Paid e una sola superficie operativa per runtime, database e coda. La slice D1 ha già superato isolamento tenant, grant atomici, dataset sintetico e query di pagina locali. Nell'account Cloudflare non esistono risorse FiscalBay e le risorse osservate appartengono a CF-Ready.

**Blocco:** lo stato corrente del piano Workers Paid va riletto prima di un'eventuale attivazione o rinnovo. Restano da chiudere callback HTTPS, trasporto email, quattro login reali, seller OAuth eBay, payload read-only autorizzato, recovery operativo, Managed Payments e vincoli legali preliminari. Il via owner di fine M0 verrà richiesto dopo queste prove. M1 resta ferma.
